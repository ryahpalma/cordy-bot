...
if (pairingCode && !conn.authState.creds.registered) {
  ...
  if (!!phoneNumber) {
    ...
    if (!Object.keys(PHONENUMBER_MCC).some(v => phone.startsWith(v))) {
      console.log(chalk.bgBlack(chalk.redBright('Start with the country code of your WhatsApp number. Example: +5219992095479')));
      process.exit(0);
    }
  } else {
    phone = await question(chalk.bgBlack(chalk.greenBright(`Please write your WhatsApp number. Example, +5219992095479: `)));
        ...
        stat(filePath, (err, stats) => {
          ...
          if (stats.isFile() && stats.mtimeMs < oneHourAgo && file !== 'creds.json') {
            unlinkSync(filePath, err => {
              if (err) throw err;
              console.log(chalk.bold.green(`File ${file} erased successfully`));
            });
          } else {
            console.log(chalk.bold.red(`File ${file} not erased ` + err));
          }
        });
      });
    });
