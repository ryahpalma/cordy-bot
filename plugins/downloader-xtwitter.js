...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  if (!text) throw `Envie o link do vídeo ou foto do X/Twitter`;
  if (enviando) return;
  enviando = true;
  try {
    ...
    enviando = false;
    throw 'Erro ao tentar acessar o servidor';
    return;
  }
};
...
const TwitterDL = async (url) => {
  return new Promise(async (resolve, reject) => {
    ...
    const response = await axios.default(`https://info.tweeload.site/status/${id[1]}.json`, {
      method: 'GET',
      headers: {
