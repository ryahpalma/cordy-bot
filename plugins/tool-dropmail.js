...
const handler = async (m, {
  ...
  if (!lister.includes(feature)) return m.reply('*Envie:*\n_' + usedPrefix + command + ' create_\n\n*Selecione algum desses*\n' + lister.map((v, index) => ' -> ' + v).join('\n'));
  ...
  if (lister.includes(feature)) {
    if (feature == 'create') {
      try {
        ...
        conn.dropmail[id] = [
          await m.reply('*E-mail:*\n' + eml[0] + '\n\n*Expira em:* ' + msToTime(timeDiff) + '\n\nEnvie: _' + usedPrefix + command + ' list_ para listar a caixa de mensagens'),
          eml[0],
          eml[1],
          eml[2],
        ];
    ...
    if (feature == 'list') {
      if (!conn.dropmail[id]) return m.reply('Sem mensagens, crie um novo e-mail primeiro\nEnvie: _' + usedPrefix + command + ' create_');
...
*Mensagem* : ${v.text}
   `.trim();
        ...
        await m.reply(teks || '*Vazio*' + '\n\nEnvie ' + usedPrefix + command + ' delete* para limpar a caixa de entrada_');
      } catch (e) {
        await m.reply(e);
      }
    }
    if (feature == 'delete') {
      if (!conn.dropmail[id]) return m.reply('Sem mensagens disponíveis para deletar');

      try {
        delete conn.dropmail[id];
        await m.reply('Caixa de entrada deletada com sucesso');
      } catch (e) {
        await m.reply(e);
      }
    }
